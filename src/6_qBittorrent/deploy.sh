...
# FIXED: Name of the shared Docker network that core services communicate over.
# Must match the network used by core-qbittorrent and other mesh services.
...
# FIXED: ensure the shared core Docker network exists so inter-service
# hostname resolution (e.g. core-qbittorrent) works across compose projects.
ensure_core_network() {
  if ! sudo docker network inspect "${CORE_NETWORK}" >/dev/null 2>&1; then
    log "Creating shared Docker network: ${CORE_NETWORK}"
    sudo docker network create "${CORE_NETWORK}" || fail "Failed to create Docker network: ${CORE_NETWORK}"
  fi
}
...
networks:
  core_network:
    ...
    name: ${CORE_NETWORK}
EOF
# FIXED: ^ config volume now targets /root/.config/Seanime (correct path for
# the default/root image variant) instead of /config which Seanime ignores.
# FIXED: ^ networks block added so core-qbittorrent hostname resolves inside
# the container via the shared mesh network.
}
...
# FIXED: ensure shared Docker network exists before writing compose and
# starting containers so core-qbittorrent is reachable by hostname.
log "[pre] Ensuring shared core Docker network exists"
ensure_core_network
