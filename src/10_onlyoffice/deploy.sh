...
services:
  onlyoffice:
    ...
    environment:
      - TZ=${TZ:-UTC}
      - APP_URL_PORTAL=https://${DOMAIN}
    volumes:
      - ${DATA_DIR}:/var/www/onlyoffice/Data
    ports:
      - "127.0.0.1:${PUBLISHED_HTTP_PORT}:5000"
    network_mode: bridge
EOF
...
ensure_value BASIC_AUTH_USER "Enter Basic Auth username for ${DOMAIN} ingress"
ensure_secret_value BASIC_AUTH_PASS "Enter Basic Auth password for ${DOMAIN} ingress"
...
log "Installing nginx, mkcert, curl, ca-certificates, apache2-utils..."
sudo apt install -y nginx mkcert curl ca-certificates apache2-utils
...
log "Directories created: ${INSTALL_DIR}, ${DATA_DIR}"

log "[3/8] Writing container runtime definition"
write_compose_file

log "[4/8] Starting OnlyOffice container"
cleanup_previous_runtime
...
log "Waiting for OnlyOffice to initialise (DocSpace cold-start may take up to 3 minutes)..."
...
log "[6/8] Configuring Nginx ingress with HTTP Basic Auth for ${DOMAIN}"
log "Writing htpasswd entry for user ${BASIC_AUTH_USER} to ${HTPASSWD_FILE}..."
echo "${BASIC_AUTH_PASS}" | sudo htpasswd -i -c "${HTPASSWD_FILE}" "${BASIC_AUTH_USER}"
sudo chmod 640 "${HTPASSWD_FILE}"
log "htpasswd file written and secured at ${HTPASSWD_FILE}"
...
log "[7/8] Validating Nginx ingress configuration"
...
log "Testing ingress at https://${DOMAIN}/ (expecting 401 from Basic Auth gate)..."
...
log "Ingress check:     curl -k -u <user>:<pass> https://${DOMAIN}/"
